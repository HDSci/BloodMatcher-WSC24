...
def bloodgroup_frequency(antigens_info: dict, pop_dist: np.ndarray):
    ...
    antigen_list = [np.array(antigens_info.get(bg).get('antigens'))[
        antigens_incl[i]] for i, bg in enumerate(blood_groups)]
    ...
    for k, antigen_dist in enumerate(antigen_dists):
        ...
        if np.size(phens, 0) == 0:
            ...
            freq = np.kron(freq, np.ones(
                [np.size(freq_vec, 0), 1])) * np.kron(np.ones([np.size(freq, 0), 1]), freq_vec)
